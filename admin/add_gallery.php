...
$connectionobj->set_charset("utf8mb4");
session_start();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['newImagesToAlbum'])) {
        ...
        $albumName_en = $_POST['albumName_en'];
        ...
        for ($i = 0; $i < $countfiles; $i++) {
            ...
            if (move_uploaded_file($fileUploadTmp, $targetFilePath)) {
                ...
                $sql = "INSERT INTO gallery_images (album,album_en, image_url) VALUES (?,?,?)";
                $stmt = $connectionobj->prepare($sql);
                $stmt->bind_param("sss", $albumName,$albumName_en, $sqlfileurl);
                ...
                echo "បរាជ័យក្នុងការផ្លាស់ទីឯកសារ។"; // Failed to move file in Khmer
            }
        }
        echo '<script>alert("រូបភាពថ្មីត្រូវបានបន្ថែមដោយជោគជ័យ"); // New images added successfully in Khmer
    ...
    // Set charset to utf8mb4 to support Khmer text
    $connectionobj->set_charset("utf8mb4");

    if (isset($_POST["submit_new_album"])) {
        // Retrieve from album name field
        $album_name = $_POST["album_name"];
        $album_name_en = $_POST["album_name_en"];
        // Insert data into the album table and create new table according to album name field
        $sql = "INSERT INTO gallery_album (album_name,album_name_en) VALUES (?,?)";

        $stmt = $connectionobj->prepare($sql);

        $stmt->bind_param("ss", $album_name,$album_name_en);

        if ($stmt->execute()) {
            echo '
            <script>
            alert("អាល់ប៊ុមថ្មីត្រូវបានបង្កើត"); // New Album is Created in Khmer
            window.location.replace("add_gallery.php");
            
            </script>';
        } else {
            echo "កំហុស: " . $stmt->error; // Error in Khmer
        }
...
<head>
    ...
    <title>Add Gallery | Monisakor primary school</title>
    ...
    <section class="text-gray-600 body-font">
        <div class="container px-5 py-10 mx-auto">
            <div class="flex flex-col text-center w-full mb-5">
                <h1 class="sm:text-3xl text-2xl font-bold title-font mb-4 text-gray-900 dark:text-white">បន្ថែមវិចិត្រសាល</h1>
                <p class="text-sm md:text-base lg:w-2/3 mx-auto leading-relaxed text-gray-900 dark:text-white">
                    សួស្តី! សង្ឃឹមថាអ្នកសុខសប្បាយ! អ្នកអាចជួយខ្ញុំសម្រាប់គេហទំព័រសាលាបានទេ?
                    យើងកំពុងបង្ហាញ [ព្រឹត្តិការណ៍/គម្រោង] ថ្មីៗ ហើយត្រូវការវិចិត្រសាលស្អាតៗមួយ។
                    ចូលទៅផ្ទាំងគ្រប់គ្រង សូមចុច "វិចិត្រសាល" ហើយបង្កើតអាល់ប៊ុមថ្មី។ បញ្ចូលរូបភាព
                    កំណត់ឲ្យអាចមើលឃើញ ហើយអស់បញ្ហា។ ប្រាប់ខ្ញុំប្រសិនបើអ្នកត្រូវការជំនួយណាមួយ! សូមអរគុណ!
                </p>
            </div>
                    ...
                    <form class="space-y-4" action="" method="POST">
                        <div>
                            <label for="album_name"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> ឈ្មោះអាល់ប៊ុម</label>
                            <input type="text" name="album_name" id="album_name"
                                ...
                                placeholder="កិច្ចប្រជុំសាលា" required>

                                <label for="album_name_en"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> Album Name</label>
                            <input type="text" name="album_name_en" id="album_name_en"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-[#181818] dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                placeholder="School Meeting" required>
                        </div>


                        <button type="submit" name="submit_new_album"
                            class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">បង្កើត</button>
    ...
    <div class="mx-10 mb-10" id="accordion-color" data-accordion="collapse"
        ...
        if ($totalAlbums > 0) {
            while ($row = mysqli_fetch_assoc($albums)) {
                ...
                $album_name_en = $row["album_name_en"];
        ?>
                <h2 id="accordion-color-heading-<?= $albumId ?>">
                    <button type="button"
                        class="flex items-center justify-between w-full p-5 font-bold rtl:text-right text-gray-500 border border-b-0 border-gray-200 focus:ring-4 focus:ring-gray-200 dark:focus:ring-[#1e1e1e] dark:border-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:bg-[#1e1e1e] focus:dark:bg-[#181818] dark:hover:bg-[#1e1e1e]"
                        data-accordion-target="#accordion-color-body-<?= $albumId ?>" aria-expanded="false"
                        aria-controls="accordion-color-body-<?= $albumId ?>">
                        <span><?= $album_name ?>_<?= $album_name_en ?></span>
                ...
                <div id="accordion-color-body-<?= $albumId ?>" class="hidden" aria-labelledby="accordion-color-heading-<?= $albumId ?>">
                    <div class="p-5 border border-b-0 border-gray-200 dark:border-gray-700">
                        <div class="flex justify-center m-5">
                            <button data-modal-target="addImagestoAlbum<?= $albumId ?>" data-modal-toggle="addImagestoAlbum<?= $albumId ?>"
                            ...
                            <button data-modal-target="deleteModal<?= $albumId ?>" data-modal-toggle="deleteModal<?= $albumId ?>"
                        ...
                        <div id="addImagestoAlbum<?= $albumId ?>" tabindex="-1" aria-hidden="true"
                            class="fadeIn hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-[#1e1e1e]">
                                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                                            បន្ថែមរូបភាពទៅ <?= $album_name ?><?= $album_name_en ?>
                                        </h3>
                                        <button type="button"
                                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                            data-modal-hide="addImagestoAlbum<?= $albumId ?>">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                                viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                        ...
                                        <form class="space-y-4" enctype="multipart/form-data" action="" method="POST">
                                            <div>
                                                <label for="imagesSchoolMemories"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">ជ្រើសរើសរូបភាព</label>
                                                <input name="file-upload-modified[]" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-[#181818] dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file" accept="image/*" required multiple>
                                                <input name="albumName" type="hidden" value="<?= $album_name ?>">
                                                <input name="albumName_en" type="hidden" value="<?= $album_name_en ?>">
                                                <input name="albumId" type="hidden" value="<?= $albumId ?>">
                                            </div>
                                            <button type="submit" name="newImagesToAlbum"
                                                class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">ផ្ទុកឡើងរូបភាព</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="deleteModal<?= $albumId ?>" tabindex="-1" aria-hidden="true"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                    <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal<?= $albumId ?>">
                                    ...
                                    <p class="mb-4 text-gray-500 dark:text-gray-300">តើអ្នកប្រាកដថាចង់លុប <?= $album_name ?>?</p>
                        ...
                        <section class="bg-white dark:bg-[#0E0E0E]">
                            <div class="py-4 px-2 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6">
                                <div class="grid gap-2 lg:gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                                    <?php
                                    $fetch_all_images = "SELECT * FROM `gallery_images` WHERE album = ? ORDER BY id DESC";
                                    ...
                                    ?>
                                            <div class="text-center text-gray-500 dark:text-gray-400">
                                                <img style="transition: 0.3s;" class="object-cover object-center hover:object-scale-down mx-auto mb-4 w-36 h-36 rounded-lg"
                                                    src="../<?= $imagerow["image_url"] ?>" alt="cannot load image">
